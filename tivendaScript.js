...
function checkVisibility() {
    ...
    if (isElementInViewport(titleServices, 0.3)) {
    ...
    if (isElementInViewport(rightServiceContainer, 0.3)) {
    ...
    if (isElementInViewport(content2Info, 0.3)) {
...
function openMaterialInfo(button) {
...
// Function to attach click event to the list items
function attachClickEvent() {
    ...
    Array.from(matsListItems).forEach(item => {
        ...
        if (window.innerWidth <= 705) {
            ...
            item.onclick = null; // Remove the onclick if the screen is wider
        }
    });
}
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    window.addEventListener("resize", attachClickEvent);
});
...
//     // Set a minimum distance from the top for the heading
//     const minTopOffset = 250;

//     // Set a minimum distance from the top for the button
//     const minButtonOffset = 400;

//     // Set a threshold for reducing the button container size
//     const thresholdHeight = 600;

//     // Calculate desired position for the heading
//     const desiredHeadingPosition = Math.max(viewportHeight * 0.45, minTopOffset); // 45% of the viewport height or minTopOffset
//     headingContainer.style.top = `${desiredHeadingPosition}px`;

//     // Calculate desired position for the button
//     const desiredButtonPosition = Math.max(viewportHeight * 0.85, minButtonOffset); // 85% of the viewport height or minButtonOffset
//     buttonContainer.style.top = `${desiredButtonPosition}px`;

//     // Reduce the button container size if the screen height is below the threshold
//     if (viewportHeight < thresholdHeight) {
//         buttonContainer.style.transform = `scale(${viewportHeight / thresholdHeight})`;
//     } else {
//         buttonContainer.style.transform = '';
//     }
// }

// // Adjust positions on load
// window.onload = adjustPositions;
